...
@section('title', 'Postingan Mading Baru')
...
@section('content')
    <div class="reveal">
        ...
        <div class="mb-10">
            ...
            <h1 class="text-3xl font-black text-gray-900 tracking-tight">Postingan Mading Baru</h1>
            <p class="text-gray-400 text-sm mt-1">Isi form di bawah untuk menambahkan postingan ke mading digital.</p>
        </div>

        <form action="{{ route('admin.mading.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="bg-white rounded-[2rem] p-8 shadow-sm border border-gray-100 space-y-7">
                        ...
                        <div>
                            ...
                            <textarea id="content" name="content" rows="16"
                                      ...
                                      placeholder="Tulis isi postingan di sini..." required>{{ old('content') }}</textarea>
                            @error('content')
                                <p class="text-red-500 text-xs mt-1.5">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white rounded-[2rem] p-7 shadow-sm border border-gray-100 space-y-5">
                        ...
                        {{-- Kategori --}}
                        <div>
                            <label class="block text-xs font-black text-gray-400 uppercase tracking-wider mb-2" for="category">
                            ...
                            <select id="category" name="category"
                                ...
                                @foreach($categories as $key => $label)
                                    <option value="{{ $key }}" {{ old('category', 'umum') === $key ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        {{-- Nama Penulis --}}
                        <div>
                            <label class="block text-xs font-black text-gray-400 uppercase tracking-wider mb-2" for="author_name">
                        ...
                        <div>
                            ...
                            <input type="text" id="author_class" name="author_class" value="{{ old('author_class') }}"
                                   class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 focus:bg-white focus:ring-4 focus:ring-red-50 focus:border-red-500 outline-none transition-all text-sm font-medium"
                                   placeholder="XII RPL 1, Guru B. Indonesia, dll.">
                        </div>

                        {{-- Tanggal Terbit --}}
                        <div>
                            <label class="block text-xs font-black text-gray-400 uppercase tracking-wider mb-2" for="published_at">
                                Tanggal Terbit
                            </label>
                    ...
                    <div class="bg-white rounded-[2rem] p-7 shadow-sm border border-gray-100">
                        <div class="flex items-center gap-3 pb-4 border-b border-gray-50 mb-5">
                            <div class="w-1.5 h-6 bg-red-600 rounded-full"></div>
                            <h3 class="font-black text-gray-900 uppercase tracking-widest text-xs">Gambar <span class="text-gray-300 normal-case font-semibold">(opsional)</span></h3>
                        </div>
                        <div class="w-full rounded-2xl bg-gray-50 border-2 border-dashed border-gray-200
                            ...
                            <svg class="w-8 h-8 mx-auto mb-2 text-gray-300 group-hover:text-red-400 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
