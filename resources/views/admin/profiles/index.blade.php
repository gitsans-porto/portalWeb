    ...
    <div class="reveal">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @foreach($profiles as $profile)
                <div class="admin-card group hover:shadow-2xl transition-all duration-500 border-t-4 {{ $profile->section === 'tentang_sekolah' ? 'border-indigo-600' : ($profile->section === 'visi_misi' ? 'border-emerald-600' : 'border-amber-600') }}">
                    <div class="flex flex-col h-full">
                        <div class="mb-6">
                            <div class="w-14 h-14 rounded-2xl flex items-center justify-center mb-6 {{ $profile->section === 'tentang_sekolah' ? 'bg-indigo-50 text-indigo-600' : ($profile->section === 'visi_misi' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600') }}">
