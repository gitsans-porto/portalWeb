...
@section('content')
    <div class="reveal">
        ...
        <div class="mb-10 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            ...
            <a href="{{ route('admin.news.create') }}" class="inline-flex items-center gap-2 px-6 py-3 rounded-2xl bg-red-600 text-white font-bold hover:bg-red-700 transition-all shadow-lg shadow-red-200">
                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors group">
                                ...
                                <td class="px-8 py-5 text-center">
                                    ...
                                    <div class="text-[10px] text-gray-400 uppercase font-bold mt-0.5">
                                        {{ $article->published_at ? $article->published_at->format('H:i') : 'Draft' }}
                                    </div>
                                </td>
                                <td class="px-8 py-5">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('admin.news.edit', $article->id) }}" class="p-2 rounded-xl bg-gray-50 text-gray-400 hover:bg-red-50 hover:text-red-600 transition-all" title="Edit">
                                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                            </svg>
                                        </a>
                                        <form action="{{ route('admin.news.destroy', $article->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus berita ini?')">
                                            @csrf
                                            @method('DELETE')
                            ...
                            <tr>
                                <td colspan="4" class="px-8 py-12 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <a href="{{ route('admin.news.create') }}" class="text-red-600 text-sm font-black uppercase tracking-widest mt-4 hover:underline">Tulis Sekarang</a>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
