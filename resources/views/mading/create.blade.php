...
<section style="background: #f8f9fc; padding: 48px 0 60px; border-bottom: 1px solid #e2e8f0; position: relative; overflow: hidden;">
    <div style="position:absolute; top:-20px; right:-20px; width:150px; height:150px; background:rgba(220,38,38,0.03); border-radius:50%; blur:60px;"></div>
        ...
        <div style="display:grid; grid-template-columns: 1fr 400px; gap:60px; align-items: start;">
                    ...
                    <div style="display:flex; flex-direction:column; gap:32px;">
                        ...
                        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:24px;">
                            <div class="input-group">
                                <label style="display:block; font-size:0.75rem; font-weight:800; color:#1e293b; text-transform:uppercase; letter-spacing:0.05em; margin-bottom:10px;">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" name="author_class" id="author_class" value="{{ old('author_class') }}"
                                       style="width:100%; padding:14px 18px; border-radius:14px; border:1.5px solid #e2e8f0; font-size:0.95rem; font-weight:600; transition:all 0.2s;"
                            ...
                            <div style="display:flex; flex-wrap:wrap; gap:10px;">
                                @foreach($categories as $key => $label)
                                    <label style="cursor:pointer; position:relative;">
                                        <input type="radio" name="category" value="{{ $key }}" class="category-radio hidden" {{ old('category', 'pengumuman') === $key ? 'checked' : '' }} onchange="updatePreview()">
                                        <div class="category-pill" style="padding:10px 20px; border-radius:12px; border:1.5px solid #e2e8f0; font-size:0.85rem; font-weight:700; color:#64748b; transition:all 0.2s;">
                                            {{ $label }}
                                        </div>
                                    </label>
                                @endforeach
                        ...
                        <div class="input-group">
                            ...
                            @error('title') <p style="color:#ef4444; font-size:0.75rem; margin-top:8px;">{{ $message }}</p> @enderror
                        </div>
                        ...
                        <div class="input-group">
                            ...
                            <textarea name="content" id="content" rows="8" required
                                      ...
                                      placeholder="Tuliskan detail karyamu di sini..." oninput="updatePreview()">{{ old('content') }}</textarea>
                            @error('content') <p style="color:#ef4444; font-size:0.75rem; margin-top:8px;">{{ $message }}</p> @enderror
                        </div>

                        {{-- Image Upload --}}
                        <div class="input-group">
                            <label style="display:block; font-size:0.75rem; font-weight:800; color:#1e293b; text-transform:uppercase; letter-spacing:0.05em; margin-bottom:10px;">
                                Lampiran Gambar <span style="color:#94a3b8; font-weight:600; text-transform:none;">(Opsional)</span>
                            </label>
                            <div id="dropzone" style="width:100%; padding:40px; border-radius:20px; border:2px dashed #e2e8f0; background:#f8fafc; text-align:center; transition:all 0.2s; cursor:pointer;"
                                ...
                                <input type="file" name="image" id="image" accept="image/*" style="display:none;" onchange="handleImage(this)">
                                <div id="uploadPlaceholder">
                                    <svg style="width:40px; height:40px; color:#cbd5e1; margin:0 auto 16px;" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                    </svg>
                                    <p style="font-size:0.95rem; font-weight:700; color:#475569; margin-bottom:4px;">Klik atau geser gambar ke sini</p>
                                ...
                                <div id="imagePreview" style="display:none; position:relative;">
                                    <img src="" id="previewImg" style="max-height:200px; border-radius:12px; margin:0 auto;">
                                    <button type="button" onclick="removeImage(event)" style="position:absolute; top:-10px; right:50%; transform:translateX(100px); background:#ef4444; color:white; border:none; width:24px; height:24px; border-radius:50%; font-size:12px; cursor:pointer; display:flex; align-items:center; justify-content:center;">&times;</button>
                                </div>
                            </div>
                        </div>
            ...
            <div style="position:sticky; top:40px;">
                <div style="margin-bottom:20px; display:flex; align-items:center; gap:8px;">
                    ...
                    <span style="font-size:0.75rem; font-weight:800; color:#94a3b8; text-transform:uppercase; letter-spacing:0.1em;">Pratinjau Langsung</span>
                </div>
                
                {{-- Preview Card (Social Media Style) --}}
                <div style="background:white; border-radius:24px; overflow:hidden; border:1px solid #f1f5f9; box-shadow:0 10px 30px rgba(0,0,0,0.05); transition:all 0.3s;">
                    <div id="previewCardImage" style="width:100%; height:200px; background:linear-gradient(135deg,#f1f5f9,#e2e8f0); display:flex; align-items:center; justify-content:center; overflow:hidden;">
                        <svg id="previewImagePlaceholder" style="width:48px; height:48px; color:rgba(0,0,0,0.05);" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                        </svg>
                        <img src="" id="previewCardImg" style="width:100%; height:100%; object-fit:cover; display:none;">
                    </div>
                    <div style="padding:24px;">
                        <span id="previewCategory" style="background:#fee2e2; color:#dc2626; padding:4px 12px; border-radius:10px; font-size:0.65rem; font-weight:800; text-transform:uppercase; letter-spacing:0.05em; margin-bottom:12px; display:inline-block;">
                            Pengumuman
                        </span>
                        <h3 id="previewTitle" style="font-size:1.15rem; font-weight:800; color:#1e293b; margin:0 0 10px; line-height:1.3; height:3rem; overflow:hidden;">
                            Judul karyamu muncul di sini...
                        </h3>
                        <p id="previewContent" style="font-size:0.9rem; color:#64748b; line-height:1.5; margin:0 0 20px; height:2.7rem; overflow:hidden;">
                        ...
                        <div style="display:flex; align-items:center; gap:12px; padding-top:16px; border-top:1px solid #f1f5f9;">
                            <div style="width:32px; height:32px; border-radius:10px; background:linear-gradient(135deg,#dc2626,#ef4444); display:flex; align-items:center; justify-content:center; color:white; font-size:0.8rem; font-weight:900;" id="previewAvatar">
                                ?
                            </div>
                            <div>
                                <div id="previewAuthor" style="font-size:0.8rem; font-weight:800; color:#1e293b;">Nama Penulis</div>
                ...
                <div style="margin-top:24px; padding:20px; background:#f0fdf4; border-radius:16px; border:1px solid #dcfce7;">
                    <div style="display:flex; gap:12px;">
                        <svg style="width:20px; height:20px; color:#16a34a; flex-shrink:0;" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <p style="font-size:0.8rem; color:#166534; line-height:1.5; margin:0;">
                            <strong>Informasi:</strong> Postinganmu akan melalui tahap kurasi oleh admin sebelum ditampilkan secara publik. Kamu akan mendapatkan notifikasi jika sudah live.
                        </p>
                    </div>
                </div>
            </div>
