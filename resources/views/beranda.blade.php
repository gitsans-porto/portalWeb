...
@section('content')
    ...
    <section class="hero-section" id="layanan" style="background-image: url('{{ asset('images/gambarSekolah.jpeg') }}'); background-size: cover; background-position: center; background-repeat: no-repeat;">
        ...
        <div class="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 lg:py-0">
            <div class="text-center mb-12 lg:mb-16 pt-16 lg:pt-28">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-5 max-w-5xl mx-auto">
                ...
                @foreach($layananList as $index => $item)
                    @php
                        $colors = [
                            'blue' => ['bg' => 'bg-blue-500/20', 'text' => 'text-white', 'border' => 'border-blue-400/20'],
                            'green' => ['bg' => 'bg-emerald-500/20', 'text' => 'text-white', 'border' => 'border-emerald-400/20'],
                            'amber' => ['bg' => 'bg-amber-500/20', 'text' => 'text-white', 'border' => 'border-amber-400/20'],
                            'purple' => ['bg' => 'bg-purple-500/20', 'text' => 'text-white', 'border' => 'border-purple-400/20'],
                        ];
                        $c = $colors[$item->color] ?? $colors['blue'];
                    @endphp

                    <a href="{{ route('layanan.detail', $item->slug) }}"
                        class="service-card flex flex-col h-full reveal reveal-delay-{{ $index + 1 }}">

                        <div class="flex flex-col items-center text-center mb-4">
                            <div class="mb-4">
                                @if($item->icon === 'document-chart-bar')
                                    <svg class="w-10 h-10 {{ $c['text'] }}" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        ...
                        <div
                            class="flex items-center mt-auto gap-1.5 text-white text-sm font-medium opacity-80 hover:opacity-100 transition-opacity">
